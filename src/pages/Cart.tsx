  ...
  return (
    <div className="mx-auto grid max-w-6xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[1fr_320px]">
        ...
        {items.map((item) => (
          <div key={item.productId} className="flex gap-4 rounded-lg border border-border bg-card p-4">
            {item.image_url ? (
              ...
              <div className="flex h-20 w-20 items-center justify-center rounded-md bg-secondary text-primary">
                <BrandMark size={28} />
              </div>
            )}
      ...
      <aside className="h-fit rounded-lg border border-border bg-card p-5">
        ...
        <p className="mt-2 text-xs text-muted-foreground">Pagas al recoger. Aquí solo reservas el pedido.</p>
        <Link
          to="/checkout"
